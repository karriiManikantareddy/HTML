...
@Component({
  selector: 'simplifi-native-device',
  templateUrl: './simpifi-native-device.component.html',
})
export class SimplifiNativeDeviceComponent {
  loading = true;
  campaigns?: Item[];
  series;
  kpis = ['impressions', 'clicks', 'ctr', 'total_conversions'];
  ...
  constructor(dataService: SimplifiDataService,
    ...
    dataService.simplifiNativeDevice().subscribe(campaigns => {
      this.loading = false
      this.options = Object.add(this.options, {colors: variables.colors[this.theme].chart})
      this.filteredDevices = campaigns.filter(d => d.metrics.impressions.value > 1000 && d.name !== null)
      this.series = [{
        type: 'pie',
