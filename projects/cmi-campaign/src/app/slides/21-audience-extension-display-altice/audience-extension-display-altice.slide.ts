...
@Component({
  selector: 'audience-extension-display-altice',
  templateUrl: './audience-extension-display-altice.slide.html',
  styleUrls: ['./audience-extension-display-altice.slide.less']
})
export class AudienceExtensionDisplayAlticeSlide {
  loading = true
  hasAed = false
  creatives = []
  maxNameLength = 18
      ...
      .subscribe(creative => {
        if (creative) {
          this.creatives = creative.rows
            .filter(row => row.getSliceId('datasource') === 'altice')
            .map(row => ({
