...
@Component({
  selector: 'newsletters-metro-breakdown',
  templateUrl: './newsletters-metro-breakdown.slide.html',
})
export class NewslettersMetroBreakdownSlide {
  ...
  constructor(dataService: TheAdvocateDataService, variables: Variables) {
    dataService.newslettersMetroBreakdownItems().subscribe(metros => {
      this.loading = false
      this.mapSeries = [{
        data: metros
