...
export class CreativeTopComponent implements OnInit {
  ...
  constructor(
    ...
    templateConfigService.getTemplateConfigs().subscribe(templateConfigs => {
      ...
      this.impressionsGaugeOptions = Object.add(this.gaugeOptions, {colors: [(templateConfigs && templateConfigs.secondary_color) || colors.impressions, colors.secondary]})
      this.clicksGaugeOptions = Object.add(this.gaugeOptions, {colors: [(templateConfigs && templateConfigs.primary_color) || colors.clicks, colors.secondary]})
    })
  }

  ngOnInit() {
    const shareOfImpressions = this.shareOfTotals['total_line_item_level_impressions'] = <number>Object.get(this.creative, 'metrics.total_line_item_level_impressions.value') / this.totalMetrics['total_line_item_level_impressions']
    const shareOfClicks = this.shareOfTotals['total_line_item_level_clicks'] = <number>Object.get(this.creative, 'metrics.total_line_item_level_clicks.value') / this.totalMetrics['total_line_item_level_clicks']
